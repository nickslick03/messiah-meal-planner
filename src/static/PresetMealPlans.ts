...
export class PresetMealPlan {
    ...
    constructor (name: string, description: string, userSelectedMealPlan: () => UserSelectedMealsObjectType) {
        this.name = name;
    ...
    weekPrice(isDiscount: boolean) {
        return getMealTotal(this.userSelectedMealPlan(), Array(7).fill(1), isDiscount, meals);
    }
}
...
function createMealReference(location: string, name: string): MealReference {
    ...
    try {
        if (!meals.some(meal => meal.id === id))
            throw new Error(`Location ${location} and/or name ${name} are not part of the meal database.`);
    } catch (error) {
        console.log(error);
    }
    return {
